...
def generate_dag(n_nodes, p_edge):
...
def generate_dag_legacy(n_nodes, p_edge):
    G = nx.DiGraph()
    ...
    for i in range(n_nodes):
        for j in range(i + 1, n_nodes):
            np.random.seed(seed)
            adjacency_matrix[i, j] = np.random.binomial(1, p)
...
def sample_edges_and_coefficients(G, n_nodes):
    edges = list(G.edges)
    gamma = np.zeros((n_nodes, n_nodes))
    seed = 0
    for i, j in edges:
        np.random.seed(seed)
        gamma[i, j] = np.random.uniform(-1, 1)  # Sample from uniform distribution
        seed += 1
    return gamma

# Step 3: Generate data
def generate_data(G, gamma, n_nodes, n_samples, disp, nb_p):
...
# Step 3: Generate data
def generate_data_legacy(G, gamma, n_nodes, n_samples, theta):
    data = np.zeros((n_samples, n_nodes))
    mu_sum = 0
    source_list = []
    seed = 0
