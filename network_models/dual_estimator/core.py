...
class NotearsMLP(nn.Module, BaseEstimator):
    ...
    def fit(
        ...
        self.adj_d = (
            self._calculate_adj(Xd_torch, mean_effect=False).cpu().detach().numpy()
        )
        self.adj_d_mean_effect = (
            self._calculate_adj(Xd_torch, mean_effect=True).cpu().detach().numpy()
        )
        self.adj_h = (
            self._calculate_adj(Xh_torch, mean_effect=False).cpu().detach().numpy()
        )
        self.adj_h_mean_effect = (
            self._calculate_adj(Xh_torch, mean_effect=True).cpu().detach().numpy()
        )
    ...
    def _dual_ascent_step(
        ...
        def _func(flat_params: np.ndarray) -> Tuple[float, np.ndarray]:
            ...
            Xd_hat = self(Xd)
            Xh_hat = self(Xh)  # NOTE: share the parameter ??
            adj_d = self._calculate_adj(Xd,mean_effect=False)  # specific adjacency matrix
            adj_h = self._calculate_adj(Xh,mean_effect=False)  # common adjacency matrix
            h_val = self._h_func()
            loss = 0.0

            #print("Xd:",Xd.shape)
            #print("Xh:",Xh.shape)
            #print("ajd_d:",adj_d.shape)
            #print("ajd_h:",adj_h.shape)
